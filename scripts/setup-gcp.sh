#!/bin/bash
# setup-gcp.sh — One-time setup for Google Cloud e2-micro VM (Ubuntu 22.04, x86_64)
# Run as: bash setup-gcp.sh
set -e

echo "=== Personal Finance Dashboard — GCP VM Setup ==="
...
echo "      ubuntu@<vm-external-ip>:~/finance/"
echo ""
...
npx prisma generate
npx prisma db push
tsx prisma/seed.ts 2>/dev/null || true
...
echo "   - ALLOWED_ORIGIN: fill in after getting your tunnel URL below"
echo ""
